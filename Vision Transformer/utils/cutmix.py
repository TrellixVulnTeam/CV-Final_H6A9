...
def cutmix_train(train_loader, model, criterion, optimizer, epoch):
    ...
    for input, target in warp_tqdm(train_loader, True):
        ...
        # cutmix
        input, target_a, target_b, lamb, _ = cutmix_data(input, target)
...
def cutmix_pic(pic1, pic2, alpha=1.0):
    if alpha > 0:
        lamb = np.random.beta(alpha, alpha)
    else:
        lamb = 1

    bbx1, bby1, bbx2, bby2 = rand_mask(pic1.size(), lamb)
    result = pic1.clone()
    result[:, :, bbx1: bbx2, bby1: bby2] = pic2[:, :, bbx1: bbx2, bby1: bby2]
