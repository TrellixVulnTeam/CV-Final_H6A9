...
class lite_vision_transformer_with_post_layers(lvt):
    ...
    def forward_cls(self, x):
        ...
        for idx in range(len(self.post_network)):
            x = self.post_network[idx](x)
        return x

    def forward(self, x, target=None, mixup_hidden=False, mixup_alpha=2):
        if mixup_hidden is True:
            layer_mix = random.randint(0, 3)   
        
        x = self.backbone[0][0](x)
        
        if self.training and self.mixtoken:
        ...
        if self.training:  # reverse "mix token", see token labeling for details.
            ...
            if mixup_hidden == True:
                return x_cls + 0.5 * x_aux.max(1)[0], y_a, y_b, lam
...
class lvt_upsample(nn.Module):
    def __init__(self):
        super(lvt_upsample, self).__init__()
        self.lvt = lvt_cls()
        state_dict = torch.load('lvt_imagenet.pth')
        self.lvt.load_state_dict(state_dict)
        self.lvt.head = nn.Linear(256, 100)
        self.lvt.aux_head = nn.Linear(256, 100)

    def forward(self, x, target=None, mixup_hidden=False, mixup_alpha=2):
        out = F.interpolate(x, size=(224,224), mode='bilinear', align_corners=True)
        out = self.lvt(out, target, mixup_hidden, mixup_alpha)

        return out

class lvt_same_params(nn.Module):
    def __init__(self, hidden_feature=64):
        super(lvt_same_params, self).__init__()
        self.lvt = lvt_cls()
        state_dict = torch.load('lvt_imagenet.pth')
        self.lvt.load_state_dict(state_dict)
        self.lvt.head = nn.Identity()
        self.lvt.aux_head = nn.Identity()

        self.add = WideResNet(depth=10, num_classes=hidden_feature, widen_factor=9, dropRate=0)
        self.head = Mlp(256 + hidden_feature, hidden_features=512, out_features=100)

    def forward(self, x):
        out = F.interpolate(x, size=(224,224), mode='bilinear', align_corners=True)
        big = self.lvt(out)
        small = self.add(x)
        out = torch.cat((big, small), 1)
        out = self.head(out)

        return out


class lvt_same_flops(nn.Module):
    def __init__(self, hidden_feature=64):
        super(lvt_same_flops, self).__init__()
        self.lvt = lvt_cls()
        state_dict = torch.load('lvt_imagenet.pth')
        self.lvt.load_state_dict(state_dict)
        self.lvt.head = nn.Identity()
        ...
        self.add = WideResNet(depth=10, num_classes=hidden_feature, widen_factor=10, dropRate=0)
        self.head = Mlp(256 + hidden_feature, hidden_features=512, out_features=100)
