...
if __name__ == "__main__":
    ...
    work_dir = 'logs_untrainfinetune/' + work_dir
    if not os.path.exists(work_dir):
        os.makedirs(work_dir)
    # Tensorboard initialization.
    trainwriter = SummaryWriter('{}/{}'.format(work_dir, 'Train'))
    valwriter = SummaryWriter('{}/{}'.format(work_dir, 'Validation'))
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model, train_util, loss_history, trainwriter, valwriter, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir)
